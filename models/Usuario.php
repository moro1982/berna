...
class Usuario extends ActiveRecord {
    ...
    // Revisa si usuario ya existe
    public function existeUsuario() {
        $query = "SELECT * FROM " . self::$tabla . " WHERE email = '" . $this->email . "' LIMIT 1";

        $resultado = self::$db->query($query);

        // Revisar que la cantidad de resultados sea distinta de 0 (existe) y guardar la alerta
    ...
    public function checkPasswordAndConfirmado($password) {
